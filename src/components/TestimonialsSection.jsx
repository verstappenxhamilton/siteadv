...
import { QuoteIcon } from './icons/Icons.jsx';

const TESTIMONIALS = [
  {
    quote:
      'Incrível eficiência e profissionalismo. Resolveram meu caso com uma agilidade que eu não esperava. Recomendo a todos.',
    isFeatured: true,
  },
  {
...
const TestimonialCard = ({ quote, authorImage, isFeatured = false }) => {
  const emphasisClasses = isFeatured
    ? 'bg-[#0D1B2A] text-white border-2 border-[#B98F58] transform md:scale-105 z-10'
    : 'bg-white text-gray-700 border';

  return (
    <div className={`p-8 rounded-lg shadow-lg flex flex-col h-full transition-shadow duration-300 hover:shadow-xl ${emphasisClasses}`}>
      <QuoteIcon className="w-12 h-12 mb-4 text-[#B98F58]" />
      ...
      {authorImage && (
        <div className="flex items-center justify-center mt-auto">
          <img
            src={authorImage}
            alt="Cliente"
            className="w-14 h-14 rounded-full object-cover border-2 border-[#B98F58]"
          />
        </div>
      )}
    </div>
...
const TestimonialsSection = () => (
  <section
    id="testimonials"
    className="py-20 bg-gray-100 fade-in-section"
    style={{
      backgroundImage:
  ...
  >
    <div className="container mx-auto px-6">
      <h2 className="text-4xl font-bold text-center text-gray-800 mb-12 uppercase tracking-wide">
