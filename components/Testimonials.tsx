...
const ModernTestimonialCard: React.FC<TestimonialCardProps> = ({ quote, authorImage, isFeatured = false }) => (
...
const Testimonials: React.FC<{ theme: HomeThemeOption }> = ({ theme }) => {
  if (theme === "legacy") {
    return (
      <section
        id="testimonials"
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 items-stretch">
            {testimonialsData.map((testimonial, index) => {
              const cardClasses = testimonial.isFeatured
              ...
              return (
                <div
                  key={index}
                  className={`p-8 rounded-lg shadow-lg flex flex-col h-full transition-shadow duration-300 hover:shadow-xl ${cardClasses}`}
                >
                  <QuoteIcon className="w-12 h-12 mb-4 text-[#B98F58]" />
                  <p className="italic text-base mb-6 flex-grow">"{testimonial.quote}"</p>
    ...
    <section id="testimonials" className="home-testimonials fade-in-section">
      <div className="container">
        ...
        <div className="home-testimonials__grid">
          {testimonialsData.map((testimonial, index) => (
            <ModernTestimonialCard key={index} {...testimonial} />
          ))}
        </div>
      </div>
    </section>
