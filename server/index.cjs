...
app.get('/api/videos', (req, res) => {
  ...
  fs.readdir(publicVideosDir, (err, files) => {
    if (err) {
      console.error('Error reading public/videos directory:', err);
      return res.status(500).json({ error: 'failed_to_read_videos_directory' });
    }
    const videoFiles = files
      .filter(file => file.endsWith('.mp4'))
      .sort(); // Sort to ensure a consistent order
    res.json(videoFiles.map(file => `/videos/${file}`)); // Return paths relative to the public root
  });
});
