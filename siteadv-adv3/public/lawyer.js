...
(() => {
  ...
  function appendReport(r) {
    ...
    const date = new Date(r.timestamp).toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' });
    
    const name = r.name || 'Não informado';
    const email = r.answers?.email || 'Não informado';
    const phone = r.answers?.phone || 'Não informado';
    const cpf = r.answers?.cpf || 'Não informado';

    let reportHtml = (r.text || '')
        .replace(/### (.*)/g, '<h3>$1</h3>')
        .replace(/\n/g, '<br>');

    div.innerHTML = `
      <div class="report-card-header d-flex justify-content-between align-items-center">
        <span class="fw-bold"><i class="bi bi-calendar-event me-2"></i>${date}</span>
      </div>
      <div class="report-card-body">
        <div class="client-info row mb-3">
          <div class="col-md-6">
            <p><i class="bi bi-person-fill me-2"></i><strong>Nome:</strong> ${name}</p>
            <p><i class="bi bi-envelope-fill me-2"></i><strong>Email:</strong> ${email}</p>
          </div>
          <div class="col-md-6">
            <p><i class="bi bi-telephone-fill me-2"></i><strong>Telefone:</strong> ${phone}</p>
            <p><i class="bi bi-person-vcard-fill me-2"></i><strong>CPF:</strong> ${cpf}</p>
          </div>
        </div>
        <details>
          <summary>Ver Detalhes do Relatório</summary>
