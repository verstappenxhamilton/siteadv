...
const { z } = require('zod');
const { generate } = require('./providers');\n\nconst projectRoot = path.resolve(__dirname, '..');\nconst dataDir = path.join(projectRoot, 'server-data');\nconst videosDir = path.join(dataDir, 'videos');\nconst publicDir = path.join(projectRoot, 'public');\nconst legacyContentPath = path.join(projectRoot, 'siteadv-adv2.8', 'public', 'content.json');\nconst contentPath = path.join(dataDir, 'content.json');\n\nif (!fs.existsSync(dataDir)) fs.mkdirSync(dataDir, { recursive: true });\nif (!fs.existsSync(videosDir)) fs.mkdirSync(videosDir, { recursive: true });\nif (!fs.existsSync(contentPath)) {\n  let seed = {};\n  if (fs.existsSync(legacyContentPath)) {\n    try {\n      seed = JSON.parse(fs.readFileSync(legacyContentPath, 'utf8'));\n    } catch (err) {\n      console.warn('Falha ao carregar content.json legacy:', err.message);\n    }\n  }\n  fs.writeFileSync(contentPath, JSON.stringify(seed, null, 2), 'utf8');\n}\n
...
(async () => {
  ...
  server.listen(PORT, HOST, () => {
    ...
    console.log(Acesse via IP local: ://<SEU_IP_LOCAL>:);
    if (!usingHttps) {
      console.log('Aviso: HTTP em IP (ex.: 192.168.x.x) pode bloquear camera/microfone. Configure HTTPS local.');
    }
  });
})();
